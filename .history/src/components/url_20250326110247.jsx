...
function Url () {
    ...
    const [qrValue, setQrValue] = useState("https://exemple.com"); // URL par défaut
  const qrRef = useRef(null); // Référence pour le QR Code
    ...
    const handleImageUpload = (event) => {
      ...
      const downloadSVG = () => {
        ...
        svg.setAttribute("xmlns", "http://www.w3.org/2000/svg"); // Assurer la validité du fichier
      ...
      const downloadPNG = () => {
        ...
        const size = 300; // Taille du QR Code en pixels
        canvas.width = size;
        canvas.height = size;
        const ctx = canvas.getContext("2d");
    
        // Dessiner le QR Code sur un canvas temporaire
        const img = new Image();
